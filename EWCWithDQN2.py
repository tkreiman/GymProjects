...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 11 14:28:14 2017
...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, replay_memory, action_indices, use_pretty_tensor=False, load_checkpoint=False):
        ...
        for i in range(len(self.var_list)):
            ...
            if i == 3:
                self.frozen_layer = layer_names[3]
                continue
        ...
        self.train_step = self.adam_train_step
    ...
    def optimize_adam(self, learning_rate, batch_size=128, max_epochs=10.0, min_epochs=1.0, loss_limit=0.015):
        ...
        for t in range(self.last_t, max_iterations + self.last_t):
            ...
            loss_val, _, loss_normal = self.session.run([self.ewc_loss, self.train_step, self.loss], feed_dict=feed_dict)
    ...
    def optimize(self, min_epochs=1.0, max_epochs=10,
        ...
        #  Computes gradients
        #state_batch, q_values_batch = self.replay_memory.random_batch()
        #feed_dict = {self.x: state_batch, self.q_values_new: q_values_batch, self.learning_rate: learning_rate}
        #print(self.session.run(tf.train.RMSPropOptimizer(self.learning_rate).compute_gradients(self.loss, self.var_list), feed_dict=feed_dict))
    ...
    def print_frozen_layer(self):
        print(self.session.run(self.get_weights_variable(self.frozen_layer)))
...
class Agent:
    ...
    def get_action(self, q_values, iteration):
        ...
        if np.random.random() < eps:
            ...
            action_q_values = np.array(q_values)[indices_of_actions]
            return np.argmax(action_q_values)

    def train_ewc(self, episodes_per_game, states_per_game):
        episodes_total = self.model.get_count_episodes()
        start_game = int(episodes_total / episodes_per_game)

        for i in range(start_game, len(self.games)):
...
if __name__ == '__main__':
    ...
    parser.add_argument("--env", required=False, default='SpaceInvaders-v0',
    ...
    parser.add_argument("--eps_per_game", required=False, type=int, default=1500,
                        help="number of episodes to run per game")

    parser.add_argument("--states_per_game", required=False, type=int, default=2000000,
                        help="number of states to run per game")
